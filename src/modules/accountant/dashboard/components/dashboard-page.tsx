...
import { mockStudents, mockRecentPayments, mockMonthlyCollection } from "@/lib/mock-data";
...
export function DashboardPage() {
  ...
  const totalOutstanding = filteredStudents.reduce((a,s)=>a+(s.totalFee-s.paidAmount),0);
  const collectionRate  = filteredStudents.length > 0
    ? Math.round((totalCollected / filteredStudents.reduce((a,s)=>a+s.totalFee,0)) * 100) : 0;
  const todayCollection = 84500;
  const monthProgress   = Math.min((mockMonthlyCollection.slice(-1)[0]?.collected||680000) / MONTH_TARGET * 100, 100);
      ...
      <main className="flex-1 p-6 space-y-5">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            { label:"This Month",        value:formatCurrency(mockMonthlyCollection.slice(-1)[0]?.collected||680000), icon:IndianRupee, color:"text-indigo-600", bg:"bg-indigo-50" },
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2 pt-2 border-t border-gray-100 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">Collected</span><span className="font-semibold text-green-600">{formatCurrency(mockMonthlyCollection.slice(-1)[0]?.collected||680000)}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Pending</span><span className="font-semibold text-amber-600">{formatCurrency(mockMonthlyCollection.slice(-1)[0]?.pending||120000)}</span></div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Card>
            ...
            <CardContent className="space-y-2 p-4">
              {mockRecentPayments.slice(0,5).map(p=>(
