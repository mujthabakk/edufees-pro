...
const INIT: Payment[] = (mockRecentPayments as any[]).map(p => ({
  ...p, division: "A", feeType: "Tuition Fee", ref: p.reference || "—", notes: "",
}));
...
export function PaymentsPage() {
  ...
  const handleSave = () => {
    ...
    const p: Payment = {
      ...
      class:pickedStudent.class, division:pickedStudent.division||"—", amount:Number(form.amount), mode:form.mode,
      date:form.date, status:"PAID", feeType:form.feeType, ref:form.ref, notes:form.notes,
    };
    setPayments(prev=>[p,...prev]);
    showToast(`✅ ${formatCurrency(p.amount)} recorded for ${p.studentName}. Invoice generated.`);
    setShowModal(false);
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <Card className="w-[380px] p-6 space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={()=>{setPayments(p=>p.filter(x=>x.id!==delPay.id));showToast("🗑️ Record deleted");setDelPay(null);}} className="bg-red-600 hover:bg-red-700 text-white">Delete</Button>
            </div>
          </Card>
        </div>
      )}
