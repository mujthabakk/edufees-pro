...
export function StudentsPage() {
  ...
  const handlePaySave = () => {
    ...
    showToast(`✅ ${formatCurrency(amt)} recorded for ${paySt.fullName}. Invoice auto-generated.`);
    setPaySt(null);
  };
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <Card className="w-[420px] p-6 space-y-4">
            ...
            <div className="flex gap-2 justify-end pt-2 border-t border-gray-100">
              ...
              <Button onClick={()=>{setStudents(p=>p.map(s=>s.id===editSt.id?{...s,...editForm}:s));showToast("✅ Student updated");setEditSt(null);}}>Save Changes</Button>
            </div>
          </Card>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <Card className="w-[380px] p-6 space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={()=>{setStudents(p=>p.filter(s=>s.id!==delSt.id));showToast(`🗑️ ${delSt.fullName} removed`);setDelSt(null);}} className="bg-red-600 hover:bg-red-700 text-white">Delete</Button>
            </div>
          </Card>
        </div>
      )}
