...
const initYears: AcademicYear[] = [
  { id:"1", label:"2025-26", startMonth:"April", startYear:"2025", endMonth:"March", endYear:"2026", status:"active",   totalStudents:675, totalGroups:7 },
  { id:"2", label:"2024-25", startMonth:"April", startYear:"2024", endMonth:"March", endYear:"2025", status:"closed",   totalStudents:612, totalGroups:7 },
  { id:"3", label:"2023-24", startMonth:"April", startYear:"2023", endMonth:"March", endYear:"2024", status:"closed",   totalStudents:580, totalGroups:6 },
  { id:"4", label:"2026-27", startMonth:"April", startYear:"2026", endMonth:"March", endYear:"2027", status:"upcoming", totalStudents:0,   totalGroups:0 },
];

const initGroups: GroupItem[] = [
  { id:"1", name:"Class 10", sections:["A","B","C"],          students:124, incharge:"Mrs. Priya Nair" },
  { id:"2", name:"Class 9",  sections:["A","B"],              students:98,  incharge:"Mr. Ravi Kumar" },
  { id:"3", name:"Class 8",  sections:["A","B","C"],          students:112, incharge:"Ms. Anjali Shah" },
  { id:"4", name:"Class 12", sections:["A","B"],              students:86,  incharge:"Dr. Suresh Menon" },
  { id:"5", name:"Class 11", sections:["Science","Commerce"], students:72,  incharge:"Mr. Arun Pillai" },
  { id:"6", name:"Class 7",  sections:["A","B","C"],          students:95,  incharge:"Mrs. Rekha Desai" },
  { id:"7", name:"Class 6",  sections:["A","B"],              students:88,  incharge:"Mr. Santosh Jha" },
];
...
export function AcademicPage() {
  ...
  const [groups, setGroups] = useState<GroupItem[]>(initGroups);
  ...
  // Quota state
  const DEFAULT_QUOTAS = ["General","Management","NRI","Sports","Staff Ward","Scholarship","SC/ST","EWS","Merit","Early Bird"];
  const [quotas, setQuotas] = useState<{ id:string; name:string; description:string }[]>(
    DEFAULT_QUOTAS.map((q, i) => ({ id:String(i+1), name:q, description:"" }))
  );
  const [quotaForm, setQuotaForm] = useState({ name:"", description:"" });
  const [editQuota, setEditQuota] = useState<{ id:string; name:string; description:string } | null>(null);
  const [showQuotaModal, setShowQuotaModal] = useState(false);
  const [deleteQuota, setDeleteQuota] = useState<{ id:string; name:string; description:string } | null>(null);
  ...
  const activateYear = (y: AcademicYear) => {
    ...
    showToast(`✅ ${y.label} is now active`);
  ...
  const saveGroup = () => {
    ...
    if (editGroup) {
      setGroups(prev => prev.map(g => g.id === editGroup.id ? { ...g, name:groupForm.name, sections, incharge:"" } : g));
      showToast("✅ Group updated");
    } else {
      setGroups(prev => [...prev, { id:String(Date.now()), name:groupForm.name, sections, students:0, incharge:"" }]);
      showToast(`✅ ${groupForm.name} added`);
    }
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <Card className="w-[380px] p-6 space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={() => { setGroups(prev => prev.filter(g => g.id !== deleteGroup.id)); showToast(`🗑️ ${deleteGroup.name} removed`); setDeleteGroup(null); }} className="bg-red-600 hover:bg-red-700 text-white">Delete</Button>
            </div>
          </Card>
        </div>
      )}
          ...
          <Card className="w-[440px] p-6 space-y-4">
            ...
            <div className="flex gap-2 justify-end pt-2 border-t border-gray-100">
              ...
              <Button onClick={() => {
                ...
                if (editQuota) {
                  setQuotas(prev => prev.map(q => q.id === editQuota.id ? { ...q, ...quotaForm } : q));
                  showToast("✅ Quota updated");
                } else {
                  setQuotas(prev => [...prev, { id:String(Date.now()), ...quotaForm }]);
                  showToast(`✅ "${quotaForm.name}" quota added`);
                }
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <Card className="w-[380px] p-6 space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={() => { setQuotas(prev => prev.filter(q => q.id !== deleteQuota.id)); showToast(`🗑️ "${deleteQuota.name}" removed`); setDeleteQuota(null); }} className="bg-red-600 hover:bg-red-700 text-white">Delete</Button>
            </div>
          </Card>
        </div>
      )}
