...
import { useCalls, useCreateCall, useDeleteCall, type CallLogDto } from "@/lib/api/hooks/useCalls";
...
function toCallLog(c: CallLogDto): CallLog {
  return {
    id: c.id,
    ...
    mobile: c.mobile,
    type: "outgoing",
    date: c.calledAt ? c.calledAt.slice(0, 10) : "",
    duration: c.duration != null ? String(Math.round(c.duration / 60)) : "—",
    summary: c.summary ?? "",
...
export function CallsPage() {
  ...
  const calls: CallLog[] = (callsQuery.data ?? []).map(toCallLog);

  const [form, setForm] = useState({
    studentId: "", studentSearch: "", admissionNo: "", mobile: "",
  ...
  const studentSearchQuery = useStudents({ search: form.studentSearch, pageSize: 5 });

  const showToast = (msg: string) => { setToast(msg); setTimeout(() => setToast(""), 3000); };

  const suggestedStudents = form.studentSearch ? (studentSearchQuery.data?.data ?? []) : [];
  ...
  const resetForm = () => setForm({ studentId:"", studentSearch:"", admissionNo:"", mobile:"", type:"outgoing", date:new Date().toISOString().slice(0,10), duration:"", summary:"", nextAction:"" });

  const handleSave = () => {
    if (!form.studentId) { showToast("⚠️ Select a student from the list"); return; }
    createCall.mutate(
      {
        studentId: form.studentId,
        calledAt: form.date ? new Date(form.date).toISOString() : undefined,
        duration: form.duration ? Number(form.duration) * 60 : undefined,
        summary: form.summary || undefined,
        nextAction: form.nextAction || undefined,
      },
      {
        onSuccess: () => { showToast(`📞 Call log saved for ${form.studentSearch}`); setShowModal(false); resetForm(); },
        onError: () => showToast("⚠️ Could not save call log"),
      },
    );
  };
          ...
          <Card className="w-[480px] p-6 space-y-4">
            ...
            <div className="space-y-3">
              <div className="relative">
                ...
                <input value={form.studentSearch} onChange={e=>{setForm(f=>({...f,studentSearch:e.target.value,studentId:""}));setStudentSugg(true);}} onFocus={()=>setStudentSugg(true)}
                ...
                {studentSugg && form.studentSearch && suggestedStudents.length > 0 && (
                  <div className="absolute z-10 top-full left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-40 overflow-y-auto">
                    {suggestedStudents.map(s=>(
                      <button key={s.id} onClick={()=>{setForm(f=>({...f,studentId:s.id,studentSearch:s.fullName,admissionNo:s.admissionNo,mobile:s.parentMobile}));setStudentSugg(false);}}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <Card className="w-[380px] p-6 space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={() => { deleteCall.mutate(delCall.id); showToast("🗑️ Call log deleted"); setDelCall(null); }} className="bg-red-600 hover:bg-red-700 text-white">Delete</Button>
            </div>
          </Card>
        </div>
      )}
