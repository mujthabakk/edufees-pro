...
import { useSendReminders } from "@/lib/api/hooks/useDomains";
...
export function RemindersPage() {
  ...
  const sendRemindersMut = useSendReminders();
  ...
  const sendReminders = (type: "WhatsApp" | "Email", ids: string[]) => {
    if (ids.length === 0) { showToast("⚠️ Select at least one student"); return; }
    const recipients = students.filter(s => ids.includes(s.id));
    sendRemindersMut.mutate({ channel: type === "WhatsApp" ? "WHATSAPP" : "EMAIL", studentIds: ids });
    const newLogs: ReminderLog[] = recipients.map(s => ({
      id: String(Date.now())+s.id, student:s.fullName, type, time:new Date().toLocaleTimeString(),
    }));
    setLogs(prev=>[...newLogs,...prev]);
    showToast(`${type==="WhatsApp"?"💬":"📧"} ${type} sent to ${recipients.length} parent(s)`);
    setSelected([]);
  };
