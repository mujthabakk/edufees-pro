...
const toInvoice = (r: InvoiceListItem): Invoice => ({
  id:r.id, invoiceNo:r.invoiceNo, studentName:r.studentName, class:r.class, division:"",
  amount:r.amount, date:r.date.slice(0,10), dueDate:"", feeType:"Tuition Fee",
  notes:"", sentEmail:!!r.sentEmail, sentWA:!!r.sentWA,
});
...
export function InvoicesPage() {
  ...
  useEffect(() => {
    if (invoicesQuery.data?.data) setInvoices(invoicesQuery.data.data.map(toInvoice));
  }, [invoicesQuery.data]);
  ...
  const suggestedStudents = studentOptions.filter(s =>
  ...
  const handleGenerate = () => {
    ...
    const match = selectedStudentId
      ...
      : studentOptions.find(s =>
          s.fullName.toLowerCase() === form.student.trim().toLowerCase() ||
          s.admissionNo.toLowerCase() === form.student.trim().toLowerCase()
        );
    if (!match) { showToast("⚠️ Select a valid student from suggestions"); return; }
    generateInvoice.mutate(
      {
        studentId: match.id,
        amount: Number(form.amount),
        notes: [form.feeType, form.notes.trim()].filter(Boolean).join(" — ") || undefined,
        paymentMode: "CASH",
      },
      {
        onSuccess: (data) => {
                  ...
                  <tr key={inv.id} className="border-b border-gray-50 hover:bg-gray-50/70">
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex gap-1">
                        ...
                        <button onClick={()=>{setInvoices(p=>p.map(i=>i.id===inv.id?{...i,sentEmail:true}:i));sendInvoice.mutate({id:inv.id,channel:"EMAIL"});showToast(`📧 Emailed to ${inv.studentName}'s parent`);}} className="p-1.5 text-gray-400 hover:text-blue-600 rounded"><Mail className="w-3.5 h-3.5" /></button>
                        <button onClick={()=>{setInvoices(p=>p.map(i=>i.id===inv.id?{...i,sentWA:true}:i));sendInvoice.mutate({id:inv.id,channel:"WHATSAPP"});showToast(`💬 WhatsApp sent`);}} className="p-1.5 text-gray-400 hover:text-green-600 rounded"><MessageCircle className="w-3.5 h-3.5" /></button>
                ...
                {studentSugg && form.student && suggestedStudents.length>0 && (
                  <div className="absolute z-10 top-full left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-40 overflow-y-auto">
                    {suggestedStudents.map(s=>(
                      <button key={s.id} onClick={()=>{upd("student",s.fullName);upd("class",s.class);upd("division",s.division||"A");setSelectedStudentId(s.id);setStudentSugg(false);}}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <Card className="w-[380px] p-6 space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={() => {
                if (!delInv) return;
                deleteInvoice.mutate(delInv.id, {
                  onSuccess: () => { showToast(`🗑️ ${delInv.invoiceNo} deleted`); setDelInv(null); },
