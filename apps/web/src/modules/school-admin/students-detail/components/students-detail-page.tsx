...
import { ArrowLeft, Edit2, Send, Phone, Mail, Download, User, BookOpen, CreditCard } from "lucide-react";
import { useStudent } from "@/lib/api/hooks/useStudents";
import { useFeeAssignments } from "@/lib/api/hooks/useFees";
...
export function StudentsDetailPage() {
  ...
  const { data: assignments = [] } = useFeeAssignments(id ?? undefined);
  const { data: paymentsData } = usePayments({ studentId: id ?? undefined, pageSize: 50 });
  ...
  const student = {
    ...
    batch: "—",
    rollNo: data.rollNo ?? "—",
    quota: data.quotaName ?? "—",
    parentMobile: data.parentMobile ?? "—",
    parentEmail: data.parentEmail ?? "—",
    whatsappNo: data.whatsappNumber ?? data.parentMobile ?? "—",
    ...
    state: data.state ?? "—",
    pin: data.pinCode ?? "—",
    loginUsername: data.admissionNo,
    loginPassword: "—",
    status: "active",
    totalFee: data.totalFee,
    paid: data.paidAmount,
    balance: data.totalFee - data.paidAmount,
  ...
  const feeHistory = (paymentsData?.data ?? []).map(p => ({
    date: p.paymentDate.slice(0, 10),
    desc: p.notes ?? "Fee Payment",
    amount: p.amount,
    mode: p.paymentMode,
    status: "PAID",
    invoice: "—",
  ...
  const pct = student.totalFee ? Math.round((student.paid / student.totalFee) * 100) : 0;
      ...
      <main className="flex-1 p-6 space-y-5">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {[
                  ...
                  ["Date of Birth", student.dob !== "—" ? new Date(student.dob).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }) : "—"],
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {[
                  ...
                  ["Quota", student.quota],
                  ["Joined On", student.joinedOn !== "—" ? new Date(student.joinedOn).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }) : "—"],
                ].map(([k, v]) => (
                  <div key={k} className="flex justify-between items-center py-1.5 border-b border-gray-50 last:border-0">
                    <span className="text-xs text-gray-500">{k}</span>
                    <span className="text-sm font-medium text-gray-900">{v}</span>
            ...
            <Card>
              ...
              <CardContent className="p-0">
                {assignedRows.length === 0 ? (
                  <p className="px-4 py-8 text-sm text-gray-400 text-center">No fee assignments yet</p>
                ) : (
        ...
        {tab === 2 && (
          <Card>
            <CardContent className="p-0">
              {feeHistory.length === 0 ? (
                <p className="px-4 py-8 text-sm text-gray-400 text-center">No payments recorded yet</p>
              ) : (
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50/50">
                  ...
                  <tbody>
                    {feeHistory.map((row, i) => (
                      <tr key={i} className="border-b border-gray-50">
                        ...
                        <td className="px-4 py-3"><StatusBadge status={row.status} /></td>
                        <td className="px-4 py-3 text-sm text-indigo-600">{row.invoice}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {tab === 3 && (
          <Card className="max-w-lg">
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-gray-500">Portal credentials are managed by the school admin.</p>
