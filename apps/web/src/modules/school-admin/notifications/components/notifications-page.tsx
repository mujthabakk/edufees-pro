...
import { useNotificationLogs, useBroadcast } from "@/lib/api/hooks/useDomains";
...
type LogRow = { id: string; studentName: string; channel: string; template: string; recipient: string; status: string; sentAt: string };

export function NotificationsPage() {
  const logsQuery = useNotificationLogs({ pageSize: 100 });
  ...
  useEffect(() => {
    if (logsQuery.data?.data) {
      setLogs(
        logsQuery.data.data.map(n => ({
          id: n.id,
          studentName: n.recipient || "—",
          channel: n.channel,
          template: n.template,
  ...
  const filtered = logs.filter(n => channel === "All" || n.channel === channel);

  const handleSend = () => {
    const channels = [form.channels.whatsapp && "WHATSAPP", form.channels.email && "EMAIL", form.channels.sms && "SMS"].filter(Boolean) as string[];
    if (channels.length === 0) { showToast("⚠️ Select at least one channel"); return; }
    broadcast.mutate(
      { template: form.template, audience: form.audience, channels, message: form.message },
      {
        onSettled: () => { showToast(`📣 Notification sent via ${channels.join(", ")} to ${form.audience}`); setShowModal(false); },
      },
    );
  };
