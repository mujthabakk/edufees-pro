...
import { useDashboardStats, useMonthlyCollection, useClassWiseCollection } from "@/lib/api/hooks/useDomains";
...
export function DashboardPage() {
  ...
  const paymentsQuery = usePayments({ pageSize: 10 });

  const stats = {
    totalStudents: statsQuery.data?.totalStudents ?? 0,
    totalCollected: statsQuery.data?.totalCollected ?? 0,
    totalPending: statsQuery.data ? statsQuery.data.totalPending + statsQuery.data.totalOverdue : 0,
    collectionRate: statsQuery.data ? Number(statsQuery.data.collectionRate.toFixed(1)) : 0,
  };
    ...
    <div className="flex flex-col flex-1">
      ...
      <main className="flex-1 p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <StatCard title="Total Students" value={stats.totalStudents.toLocaleString()} change="+48 this month" changeType="up" icon={Users} iconBg="bg-blue-50" iconColor="text-blue-600" />
          <StatCard title="Total Collected" value={formatCurrency(stats.totalCollected)} change="+12.4% vs last month" changeType="up" icon={IndianRupee} iconBg="bg-green-50" iconColor="text-green-600" />
          <StatCard title="Pending Dues" value={formatCurrency(stats.totalPending)} change="156 students" changeType="neutral" icon={AlertTriangle} iconBg="bg-yellow-50" iconColor="text-yellow-600" />
          <StatCard title="Collection Rate" value={`${stats.collectionRate}%`} change="+2.1% vs last month" changeType="up" icon={TrendingUp} iconBg="bg-indigo-50" iconColor="text-indigo-600" />
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  {["Student", "Admission No", "Amount", "Mode", "Date", "Status"].map(h => (
              ...
              <tbody>
                {recentPayments.map(p => (
                  <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/70">
                    ...
                    <td className="px-5 py-3.5 text-sm text-gray-500">{p.admissionNo}</td>
                    <td className="px-5 py-3.5 text-sm font-semibold">{formatCurrency(p.amount)}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-500">{p.paymentMode}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-400">{p.paymentDate ? p.paymentDate.slice(0, 10) : ""}</td>
