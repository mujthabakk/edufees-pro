...
} from "@/lib/api/hooks/useStudents";
import { useClasses } from "@/lib/api/hooks/useAcademic";
import type { StudentSummary } from "@edufees/shared-types";
import {
...
type Student = {
  id: string; fullName: string; admissionNo: string; class: string; division: string;
  quota: string; parentMobile: string; parentEmail: string; totalFee: number;
...
// Maps an API student summary to the shape this page's UI expects.
...
export function StudentsPage() {
  ...
  // Fetch a generous page from the API; existing client-side filters operate over it.
  ...
  const classNameToId = useMemo(() => {
    ...
    (classesData ?? []).forEach((c) => map.set(c.name, c.id));
    return map;
  }, [classesData]);
  ...
  // Sync server data into local state (local state also drives optimistic UI).
  useEffect(() => {
    if (studentsQuery.data?.data) {
      setStudents(studentsQuery.data.data.map(toUiStudent));
    }
  }, [studentsQuery.data]);
  ...
  const handleAdd = () => {
    ...
    const classId = classNameToId.get(form.class);
    if (!classId) { showToast(`⚠️ Class "${form.class}" not found in this school`); return; }
    createStudent.mutate(
      {
        fullName: form.fullName,
        admissionNo: form.admissionNo,
        classId,
        parentMobile: form.parentMobile || "0000000000",
        parentEmail: form.parentEmail || undefined,
        dateOfBirth: form.dob || undefined,
        fatherName: form.parentName || undefined,
        addressLine1: form.address || undefined,
      },
      {
        onSuccess: () => { showToast(`✅ ${form.fullName} added successfully`); setShowAdd(false); setForm({ ...emptyForm }); },
        onError: () => showToast("❌ Failed to add student"),
      },
    );
  };
  ...
  const handleEdit = () => {
    ...
    updateStudent.mutate(
      ...
      {
        onSuccess: () => { showToast(`✅ ${form.fullName} updated`); setEditStudent(null); },
        onError: () => showToast("❌ Failed to update student"),
      },
    );
  };
  ...
  const handleDelete = () => {
    ...
    const name = deleteStudent.fullName;
    deleteStudentMut.mutate(deleteStudent.id, {
      onSuccess: () => { showToast(`🗑️ ${name} removed`); setDeleteStudent(null); },
      onError: () => showToast("❌ Failed to remove student"),
    });
  };
  ...
  const handleBulkImport = () => {
    if (bulkRawRows.length === 0) { setBulkStep("done"); return; }
    bulkImport.mutate(
      { rows: bulkRawRows },
      {
        onSuccess: (res) => { showToast(`📤 Queued ${res.queued} rows for import`); setBulkStep("done"); },
        onError: () => showToast("❌ Bulk import failed to queue"),
      },
    );
  };

  const closeBulk = () => { setShowBulk(false); setBulkStep("upload"); setBulkRows([]); setBulkRawRows([]); setBulkFileName(""); };
