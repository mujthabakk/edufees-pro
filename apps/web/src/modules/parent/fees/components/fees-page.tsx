...
import { useMyStudent, useMyFees, useInitiatePayment, useVerifyPayment } from "@/lib/api/hooks/usePortal";
...
export function FeesPage() {
  ...
  const payable = totalDue - discount;
  const primaryAssignment = fees.find(f => f.netAmount - f.paidAmount > 0);

  const handlePay = async () => {
    if (!primaryAssignment || payable <= 0) return;
    setPaying(true);
    try {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{student?.fullName ?? "—"} · {student?.className ?? "—"} · 2025-26</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <Card className="p-4 bg-green-50 border-green-200">
          ...
          <p className="text-xs text-green-600 mt-1">{feeItems.filter(f => f.status === "PAID").length} items paid</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs font-medium uppercase text-gray-500">Annual Fee</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">₹{annualFee.toLocaleString()}</p>
          <div className="mt-2 h-1.5 bg-gray-200 rounded-full">
            <div className="h-1.5 bg-green-500 rounded-full" style={{ width: `${paidPct}%` }} />
          </div>
          <p className="text-xs text-gray-500 mt-1">{paidPct}% paid</p>
        </Card>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center">
          <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-2 pt-2">
              <Button className="flex-1 bg-teal-600 hover:bg-teal-700" onClick={handlePay} disabled={paying || !primaryAssignment}>
                {paying ? "Processing..." : `Pay ₹${payable.toLocaleString()}`}
              </Button>
