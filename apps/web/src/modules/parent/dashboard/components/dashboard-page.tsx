...
export function DashboardPage() {
  ...
  const feeDetails = feesData.map(f => ({
    id: f.id,
    feeType: f.feeTypeName,
    dueDate: f.dueDate.slice(0, 10),
    grossAmount: f.netAmount,
    discount: 0,
  ...
  const recentPayments = invoicesData.map(inv => ({
    id: inv.id,
    invoiceNo: inv.invoiceNo,
    feeType: inv.invoiceNo,
    amount: inv.amount,
  ...
  const totalFee = studentData?.totalFee ?? feeDetails.reduce((sum, f) => sum + f.netAmount, 0);
  const totalPaid = studentData?.paidAmount ?? feeDetails.reduce((sum, f) => sum + f.paid, 0);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 truncate">{school?.primaryEmail ?? ""} · {school?.primaryPhone ?? ""}</p>
        </div>
