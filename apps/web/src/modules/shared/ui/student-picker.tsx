...
// Maps an API student summary into the picker's flat shape.
function toPickerStudent(s: StudentSummary): PickedStudent {
  return {
    id: s.id,
    fullName: s.fullName,
...
export function StudentPicker({ value, onChange, placeholder = "Search by name or admission no...", label = "Student", required }: Props) {
  ...
  const { data } = useStudents({ pageSize: 200 });
  const students = (data?.data ?? []).map(toPickerStudent);
